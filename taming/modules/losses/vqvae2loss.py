...
class VQLPIPSWithDiscriminator(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
        ...
        if optimizer_idx == 0:
            ...
            loss = nll_loss + d_weight * disc_factor * g_loss + self.codebook_weight[0] * codebook_loss[0].mean() \
                                                                + self.codebook_weight[1] * codebook_loss[1].mean()

            log = {"{}/total_loss".format(split): loss.clone().detach().mean(),
                   "{}/quant_loss_stage2".format(split): codebook_loss[0].detach().mean(),
                   "{}/quant_loss_stage1".format(split): codebook_loss[1].detach().mean(),
